...
const PROJECTS = [
  {
    ...
    desc: "Built with Next.js + React for fast navigation, SEO-friendly Vercel hosting, and a component system that scales. Leveraged Next.js optimizations (route prefetching, automatic code-splitting, and Next/Image) to help keep an amazing PageSpeed Insights score. Optimized interactions/animation to stay smooth, and structured the page for conversion (clear CTA hierarchy, scannable sections, minimal friction).",
  ...
  },
  {
    ...
    desc: "Next.js + React to keep the UX snappy while staying SEO-friendly for discovery. Used Next.js performance features (image optimization, code-splitting, and prefetching) to help maintain an amazing PageSpeed Insights score. Motion is tuned to feel premium without hurting performance, and the layout is designed for conversions (menu clarity, reservation-first flow, strong visual hierarchy).",
  ...
  },
  {
    ...
    desc: "Next.js + React for fast page loads and reusable UI blocks while iterating on the funnel. Next.js optimizations (automatic code-splitting, prefetching, and optimized images) help keep an amazing PageSpeed Insights score even with motion. Scroll animations are implemented with a performance-first mindset, and the page structure is built to convert (tight messaging, CTA placement, reduced distraction).",
